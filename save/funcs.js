function getTextString(offset, size, data) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    console.log(`Byte ${i}: 0x${code.toString(16)} => ${getChar(code)}`);
    if (code === 0x50) break;
    output += getChar(code);
  }
  return output;
...
function setTextString(offset, size, data, code) {
  ...
  let tamanho = code.length - 1;

  if (tamanho > size) return false;

  for (let i = 0; i <= tamanho; i++) {
    console.log(`Byte ${i}: var: ${code[i]} ${getHex(code[i])} => ${datacoisa[offset + i]}`);
    //if (code[i]) break;
    output += getChar(code);
    datacoisa[offset + i] = getHex(code[i]);
  }
  return datacoisa;
}
